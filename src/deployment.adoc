...
The goal is to avoid syntax errors when importing transports, this is done by storing objects from transports and systems in git, then simulating the state after import and then verifying by running link:https://abaplint.org/[abaplint].
...
Git -> CI : Simulate queued QAS TRs
